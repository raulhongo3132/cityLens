...
function switchTab(tab) {
  ...
  const activeClasses =
    "cat-btn px-6 md:px-8 py-3 text-[14px] md:text-[18px] font-black tracking-[0.2em] uppercase transition-all duration-300 bg-[#27dae0] text-white shadow-md border-0";
  const inactiveClasses =
    "cat-btn px-6 md:px-8 py-3 text-[14px] md:text-[18px] font-black tracking-[0.2em] uppercase transition-all duration-300 bg-transparent hover:text-[#27dae0] dark:hover:text-[#27dae0] shadow-none border-0 force-black";
...
async function buscarCiudad() {
  ...
  try {
    const res = await fetch(
      `${API_URL}/city?name=${encodeURIComponent(query)}`,
    );
...
function showToast(message, type = "info") {
  ...
  const borderColor =
    type === "error" ? "border-[#ef4444]" : "border-[#27dae0]";
